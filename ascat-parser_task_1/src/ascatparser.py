...
def helper(args):
    return parser(args.caveman_segments, args.copynumber, args.normal_hets, args.depth, args.het_den_min)
...
def parser(caveman_path, copynumber_path, copynumber_normal_path, depth, het_den_min ):
    ...
    HET_DEN_MIN   = het_den_min # minimum het density (hets/bp) allowed in segment of hets (~anti low info seg)
    ...
    cp['Chromosome'] = cp['Chromosome'].astype(int)
    # load matched normal het data
    het_normal = pandas.read_csv(copynumber_normal_path, sep='\t',dtype={'#CHR': 'str'})
    ...
    for i, row in caveman_segs.iterrows():
        ...
        cpseg = cp[(cp['Position'] >= row['start_bp']) & (cp['Position'] <= row['end_bp']) &
                      (cp['Chromosome'].astype(int) == int(row['chromosome']))]

        # check for gap fraction of segment.
        if sum(cpseg['het']) < 2:
            continue
        gap=numpy.amax(numpy.diff(cpseg[cpseg['het']]['Position'] ) )
        xgap = float(gap) /row['length']
        if xgap > HET_GAP_MAX:
            continue

        nhet = sum(cpseg['het'])
        hetden = float(nhet)/row['length']
...
def create_plots(args, acs_df, cp):
    ...
    y2 = y1
    sigma_LogR = acs_df['sigma.tau'].div(acs_df['tau']*numpy.log(2))
    #hold(True)
    for i in range(acs_df['Chromosome'].shape[0]):
        p = plt.Rectangle((x1[i],y1[i]-sigma_LogR[i]/2), x2[i] - x1[i], sigma_LogR[i], alpha=0.91, edgecolor=None, facecolor=(0.5, 0.5, 0.5))
        ax1.add_patch(p)
        ax1.plot([x1[i],x2[i]],[y1[i],y2[i]], ls='-', color = 'k', linewidth = 2)
    ...
    for i in range(0,22,2):
        p = plt.Rectangle((xL[i], 0), xL[i+1]-xL[i], 1, alpha=0.1, edgecolor=None, facecolor=(0.7, 0.7, 0.7))
        ax2.add_patch(p)
    x1 = xhg38(acs_df['Chromosome'].astype(int), acs_df['Start.bp'])
    x2 = xhg38(acs_df['Chromosome'].astype(int),acs_df['End.bp'])
    y1 = list(acs_df['f'])
    ...
    #hold(True)
    for i in range(acs_df['Chromosome'].shape[0]):
        ax2.plot([x1[i],x2[i]],[y1[i],y2[i]], ls='-', color = 'k', linewidth = 2)
